use revm::{
    db::{CacheDB, EmptyDB},
    primitives::{
        address, bytes, keccak256, AccountInfo, Address, BlockEnv, Bytecode, Bytes, TxEnv, U256,
    },
    Database, Evm,
};
...
const L2_FEE_VAULT: Address = address!("530000000000000000000000000000000000000a");

fn main() {
    println!("start test eth_fee_normal");
    eth_fee_normal();
    println!("start test alt_fee_normal");
    alt_fee_normal();
}
...
fn alt_fee_normal() {
    ...
    assert!(
        erc20_value_vault.to::<u64>() == 84000,
        "Gas fees should use: 84,000"
    ); //recive gas fees: 84,000
...
fn exec_alt_fee_txn(
    ...
    let account_to = *tx.transact_to.to().unwrap();
    ...
    let token_registry_slot = U256::from(151u64).to_be_bytes_vec();
    ...
    let price_ratio_slot = U256::from(153u64).to_be_bytes_vec();
    ...
    let erc20_value_vault = evm
        ...
        .evm
        .db
        .storage(token_account, storage_key_u256_vault)
        .unwrap_or_default();
