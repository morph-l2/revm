use revm::{
    Database, Evm, db::{CacheDB, EmptyDB}, morph::erc20_fee::L2_FEE_VAULT, primitives::{AccountInfo, Address, Bytecode, Bytes, TxEnv, U256, address, bytes, keccak256}
};
...
fn main() {
    println!("start test eth_fee_normal");
    eth_fee_normal();
    println!("start test erc20_fee_normal");
    alt_fee_normal();
}
...
fn alt_fee_normal() {
    ...
    let (account_from_balance, erc20_value, erc20_balance_evm) =
...
fn exec_alt_fee_txn(
    ...
    let storage_key = keccak256(&data);
    let storage_key_u256_to = U256::from_be_bytes(storage_key.0);
    let balance_value = U256::from(100000000u64);
    let _ = cache_db.insert_account_storage(token_account, storage_key_u256_to, balance_value);
    ...
    let erc20_value_to = evm
        .context
        .evm
        .db
        .storage(token_account, storage_key_u256_to)
        .unwrap_or_default();
    println!("===>account_to_erc20_value: {:?}", erc20_value_to);
    println!("===>storage_key_u256: {:?}", storage_key_u256);

    // // Test converting storage_key_u256_to to string and back to U256
    // println!("Testing U256 to string and back");
    // let str_val = storage_key_u256_to.to_string();
    // println!("String value: {}", str_val);
    // let back_to_u256: U256 = str_val.parse().unwrap();
    // println!("Back to U256: {:?}", back_to_u256);
    // assert_eq!(back_to_u256, storage_key_u256_to, "Conversion should be consistent");
    ...
    (account_from_balance, erc20_value, erc20_balance_evm)
}
...
fn eth_fee_normal() {
    ...
    cache_db.insert_account_info(account_to, acc_info_to.clone());
    ...
    let account_to_balance = evm
        .context
        .evm
        .inner
        .db
        .load_account(account_to)
        .unwrap()
        .info
        .balance;
    println!("account_to_balance: {:?}", account_to_balance);
    ...
    assert!(
        account_to_balance.to::<u64>() == 1000,
        "account_to should receive 1000 wei."
    ); //account_to should receive 1000 wei.
}
